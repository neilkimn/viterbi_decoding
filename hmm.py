...
class HMM(object):
    ...
    def predict(self, sentence, method='most_likely', beam_search=False):
        """
        ...
        if method == 'viterbi':
            results.append(self.predict_viterbi(sentence))
        else:
            results.append(self.predict_most_likely(sentence))
        return results
    ...
    def predict_viterbi(self,sentence):
        ...
        self.graph.add_node((0, self.tags[np.argmax(viterbiProbs[0])]))
        ...
        for t in range(1,N):
            ...
            for tagIdx, tag in enumerate(self.tags):
                ...
                viterbiProbs[t][tagIdx] = emisProb * transProb * viterbiProbs[t-1][prev_tag_idx]
                viterbiBacktrace[t]
